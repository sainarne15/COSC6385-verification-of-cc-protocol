...
void StateManager::PrintProb()
{
  ...
  std::cout << "Omission Probabilities (caused by Hash Compaction):\n\n"
       << "\tPr[even one omitted state]    <= " << 1-pNO << "\n";
  if (args->main_alg.mode == argmain_alg::Verify_bfs)
    std::cout << "\tPr[even one undetected error] <= " << 1-pno << "\n"
...
void StateManager::print_capacity()
{
  if (  args->main_alg.mode == argmain_alg::Verify_dfs 
    ...
    {
      std::cout << "\nMemory usage:\n\n";
      std::cout << "\t* The size of each state is " << BITS_IN_WORLD << " bits "
...
{ 
  ...
  if (!args->no_deadlock.value && StateCmp(originalstate,curstate)==0)
    {
      std::cout << "\nStatus:\n\n";
      std::cout << "\t" << rules_fired << " rules fired in simulation in "
	   << SecondsSinceStart() << "s.\n";
  ...
  for ( what_rule=0; what_rule<numrules; what_rule++)
  {
    if (generator->Condition(what_rule) !=
      ...
      if (!fire->in(what_rule)) {
        std::cout << "Condition for rule " << what_rule << " is true ";
        std::cout << "but it is not in fire!\n";
        exit(89);
      }
      else {
	  ...
	  if (StateEquivalent(&nextstate, p))
	    {
	      ...
	      std::cout << "Rule "
		   ...
		   << " fired.\n";
              delete[] s;   // Uli: avoid memory leak
	      if (fullstate)
		std::cout << "The last state of the trace (in full) is:\n";
  ...
  if (args->print_rule.value)
    {
      ...
      std::cout << "Rules Information:\n\n";
      for (int i=0; i<RULES_IN_WORLD; i++)  
	std::cout << "\tFired " << NumTimesFired[i] << " times\t- Rule \""
	     << generator->Name(i)
	     << "\"\n";
    }
  else
	...
	if (NumTimesFired[i]==0)
	  exist = TRUE;
      if (exist)
	std::cout << "Analysis of State Space:\n\n"
	        ...
	        if (StateSet->Add(nextstate, TRUE, TRUE))
		{
 	        ...
 	        else
 		{
 		  ...
 		  if (args->verbose.value) 
 		    std::cout << "This state has been examined, try another rule.\n";
 		}
              }
              else
                if (args->verbose.value)
